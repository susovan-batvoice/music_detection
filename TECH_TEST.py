'''We build models based on the featurs in the given csv files, which contains the the
start point and the duration of each music and string'''
...
path=r'C:\Users\PC\Documents\Important documents\Batvoice Tech Test'
...
'''Below, we prepare the y values for the classifier; files[i][105] gives us the number
pronounced for the i-th audio file, where 0 <= i <= 1999. So, y stores all the numbers pronounced''' 
y=[] 
for i in range(len(files)):
    y.append(float(files[i][105]))
...
parameters = {'kernel':('rbf'), 'C':(10,1,0.25,0.5),'gamma': (0.001,'auto', 0.01, 1)}
